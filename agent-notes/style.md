...
Your strongest bias is over-commenting: you narrate your reasoning inline, and a
reviewer then deletes half of it. Code explains itself; a comment earns its
place only by stating something surprising the code cannot.

Delete module- and class-level "architecture essay" docstrings; a short glossary
of the data model or a hook table beats paragraphs of prose.
Delete step banners that narrate linear code (`# 1. update timeline`, `# 2.
handle labels`).
Keep a surprising "why", but shrink it to the one surprising fact, on one line.
Before: a five-line note on legacy embeddings. After: `# Legacy blobs wrote []
for silence padding turns.`
Delete meta-justifications — comments defending why a thing is shared, canonical,
or "can't drift".
...
Inline a single-use helper, but extract genuinely-shared logic to a module-level
free function (drop `self`, pass what it needs) — don't trap it as a method.
Name for the true contract, not the first use: `materialize` →
`backfill_uncopied`, `DATA_TABLE_PREFIX` → `STEPPER_TABLE_PREFIX`.
Use bare `_` for every unused tuple element: `(d, m, l, _, n, _, _)`.
Inlining a helper isn't done until its tests call the surviving public API, not
the deleted private one.
